      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        <Line
          data={{
            labels,
            datasets: [
              { label: '우안(OD)', data: sorted.map(e => { const v = parseFloat(e.serOD); return isNaN(v) ? null : -v }), borderColor: '#3B82F6', tension: 0.4, pointRadius: 4, fill: false },
              { label: '좌안(OS)', data: sorted.map(e => { const v = parseFloat(e.serOS); return isNaN(v) ? null : -v }), borderColor: '#F97316', tension: 0.4, pointRadius: 4, fill: false },
            ],
          }}
          options={{
            responsive: true,
