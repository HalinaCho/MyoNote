...
export default function ClinicPatientsAllPage() {
  ...
  useEffect(() => {
    if (!hospital) return
    Promise.all([q.fetchPatientRoster(hospital.id), q.fetchOverduePatients(hospital.id)])
      .then(([roster, od]) => { setPatients(roster); setOverdue(od) })
      ...
      {patients.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
          <div className="flex items-center gap-3 px-4 py-2 text-[11px] font-medium text-gray-400">
            ...
            <div className="w-24 text-right">마지막 검사</div>
            <div className="w-24 text-right">다음 예약</div>
            <div className="w-4" />
          </div>
          {filtered.map(p => (
            <Link key={p.childId} href={`/clinic/patients?child=${p.childId}`}
              ...
              {/* 절대 날짜는 마우스를 올렸을 때 — 평소엔 "얼마나 됐나/남았나"만 읽으면 된다 */}
              <div className="w-24 text-right text-xs text-gray-400" title={p.lastExamDate ?? ''}>
                {pastLabel(p.lastExamDate) || '-'}
              </div>
              <div className={`w-24 text-right text-xs ${dueClass(p.nextAppointment)}`}
